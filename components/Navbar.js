  ...
  return (
    <>
      <nav className={`sticky top-0 dark:bg-black dark:text-white dark:border-b  bg-white z-50 shadow-sm sm:px-6 sm:py-2 p-2 flex items-center justify-between sm:grid sm:grid-cols-12 font-semibold uppercase`}>
        ...
        {/* Desktop View */}
        <div className={`hidden dark:bg-black dark:text-white sm:col-span-8 sm:flex sm:items-center sm:justify-end sm:gap-8`}>
          {darkMode ? (
      ...
      {!sidebar && (
        <div
          className={` dark:bg-black dark:text-white sm:hidden sticky top-[72px] w-full z-50 bg-white flex flex-col items-end shadow-md font-semibold`}
        >
          <Link
            href="/"
            onClick={() => setSidebar(true)}
